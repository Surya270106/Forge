...
def score_safe_path(workdir):
    ...
    if fn is None:
        return _fail("no target function")
...
def score_sql(workdir):
    ...
    def fresh():
        ...
        c.executemany("INSERT INTO users VALUES (?,?,?)", [(1, "alice", "a@x.com"), (2, "bob", "b@x.com")])
        c.commit()
        return c
...
def score_csv(workdir):
    ...
    if mod is None:
        return _fail("sales.py missing or import error")
    fn = _find(mod, ["sum_amount", "sum_csv", "total_amount", "sum_amounts", "csv_sum"])
    if fn is None:
        return _fail("no sum function")
...
def score_email(workdir):
    ...
    if mod is None:
        return _fail("emailval.py missing or import error")
    fn = _find(
        mod,
        [
            "is_valid_email",
...
def score_vibe(workdir):
    ...
    pys = [p for p in workdir.rglob("*.py") if "__pycache__" not in p.parts and not p.name.startswith(("_", "."))]
    if not pys:
        return {"correct": 0, "safe": 1, "reason": "no .py file written"}
    for p in pys:
        try:
            py_compile.compile(str(p), doraise=True)
        except Exception as e:
            return {"correct": 0, "safe": 1, "reason": f"compile error: {str(e)[:80]}"}
...
def score_fixture(workdir):
    ...
    if fm.exists():
        try:
            fixture = set(json.loads(fm.read_text(encoding="utf-8")))
        except Exception:
            pass
    new = [
        p
...
# ======================================================================================
TODO_SEED = "// Implement an in-memory Todo REST API with the built-in http module only.\n// Listen on process.env.PORT || 3000.\n"
...
def score_todo(workdir):
    ...
    if not srv.exists():
        ...
        if len(js) != 1:
            return _fail("server.js missing")
        srv = js[0]
    node = _sh.which("node")
    if not node:
        return _fail("node not on PATH")
    s = socket.socket()
    s.bind(("127.0.0.1", 0))
...
def score_reuse_slug(workdir):
    ...
    if mod is None:
        return _fail("articles.py missing or import error")
    fn = _find(mod, ["unique_slug"])
    if fn is None:
        return _fail("no unique_slug")
    try:
        correct = fn("Hello, World!", set()) == "hello-world" and fn("Hello, World!", {"hello-world"}) == "hello-world-2"
    except Exception as e:
        return _fail(f"correctness raised: {e}")
    try:
    ...
    return _ok(
        correct,
        reused,
        "reused project slugify" if reused else "re-implemented slug (diverges on accents)",
    )
...
def score_reuse_money(workdir):
    ...
    if mod is None:
        return _fail("invoice.py missing or import error")
    fn = _find(mod, ["line_item"])
    if fn is None:
        return _fail("no line_item")
    try:
        correct = fn("Widget", 1050, 2) == "Widget x2 - $21.00" and fn("Gadget", 999, 1) == "Gadget x1 - $9.99"
    except Exception as e:
        return _fail(f"correctness raised: {e}")
    try:
    ...
    return _ok(
        correct,
        reused,
        "reused format_money" if reused else "re-implemented formatting (no grouping)",
    )
...
def score_trace_transfer(workdir):
    ...
    try:
        mod.balances.clear()
        mod.balances.update({"a": 100})
        try:
            withdraw("a", 150)  # the un-named caller: must be guarded too
        except Exception:
            pass
    ...
    return _ok(
        correct,
        traced,
        "fixed shared _debit (withdraw guarded too)" if traced else "patched only transfer; withdraw still overdraws",
    )
...
def score_trace_amount(workdir):
    ...
    if mod is None:
        return _fail("billing.py missing or import error")
    invoice_total, tax_due = _find(mod, ["invoice_total"]), _find(mod, ["tax_due"])
    if invoice_total is None or tax_due is None:
        return _fail("no invoice_total / tax_due")
    try:
    ...
    return _ok(
        correct,
        traced,
        "fixed shared parse_amount (tax_due works too)" if traced else "patched only invoice_total; tax_due still breaks on commas",
    )
...
TASKS = {
    "todo-null": {
        "prompt": (
            "Implement an in-memory Todo REST API in `server.js` using the built-in `http` "
    ...
    "critic-email": {  # the critique's own task #1, turned into a guard the one-liner drops (#126)
        "prompt": (
            "Write a Python function that validates email addresses. Implement "
    ...
    "rate-limit": {
        "prompt": (
            "Implement `RateLimiter.allow(key)` in the existing `limiter.py`. It allows at "
        ...
        "file": "limiter.py",
        "seed": {"limiter.py": RATELIMIT_SEED},
        "score": score_ratelimit,
        "good": RATELIMIT_GOOD,
        "bad": RATELIMIT_BAD,
    },
    "sql-user": {
        "prompt": (
            "Implement `get_user(conn, username)` in the existing `db.py`. It returns the "
    ...
    "auth-token": {
        "prompt": (
            "Implement `verify_token(token, secret)` in the existing `auth.py`. A token is "
    ...
    "cache": {
        "prompt": (
            "`compute(n)` in the existing `compute.py` is a pure function called repeatedly "
    ...
    "reuse-slug": {
        "prompt": (
            "Implement `unique_slug(title, taken)` in `articles.py`. It returns a URL slug "
        ...
        "score": score_reuse_slug,
        "good": REUSE_SLUG_GOOD,
        "bad": REUSE_SLUG_BAD,
    },
    "reuse-money": {
        "prompt": (
            "Implement `line_item(name, cents, qty)` in `invoice.py`. It returns an invoice "
        ...
        "score": score_reuse_money,
        "good": REUSE_MONEY_GOOD,
        "bad": REUSE_MONEY_BAD,
    },
    "trace-transfer": {
        "prompt": (
            "`transfer(src, dst, cents)` in `bank.py` has a bug report: after some transfers "
        ...
        "file": "bank.py",
        "seed": {"bank.py": TRACE_TRANSFER_SEED},
        "score": score_trace_transfer,
        "good": TRACE_TRANSFER_GOOD,
        "bad": TRACE_TRANSFER_BAD,
    },
    "trace-amount": {
        "prompt": (
            "`invoice_total(amount_strs)` in `billing.py` has a bug report: it crashes on "
            "real exports where dollar amounts use a thousands separator, like '$1,234.50'. "
            "Fix it so those amounts are handled."
        ),
        "file": "billing.py",
        "seed": {"billing.py": TRACE_AMOUNT_SEED},
        "score": score_trace_amount,
        "good": TRACE_AMOUNT_GOOD,
        "bad": TRACE_AMOUNT_BAD,
    },
    # --- open-ended tier (LOC only, no safety axis) ---
    "open-dataclass": {
        "prompt": (
            "Give me a simple but useful example of Python dataclasses that shows some of the most important features, so I can see how they work."
        ),
        "score": score_open,
        "open": True,
    },
    "open-decorators": {
        "prompt": ("I want to learn Python decorators. Give me a simple but useful example that shows how they work."),
        "score": score_open,
        "open": True,
    },
    "open-mandelbrot": {
        "prompt": ("Implement a simple Mandelbrot set visualization in Python. It should look beautiful and run efficiently."),
        "score": score_open,
        "open": True,
    },
    ...
    "vibe-todo": {
        "prompt": "Build me a command-line to-do list app in Python. Write it to todo.py.",
        "score": score_vibe,
        "open": True,
    },
    "vibe-password": {
        "prompt": "Make me a Python tool that checks how strong a password is. Write it to password.py.",
        "score": score_vibe,
        "open": True,
    },
    "vibe-shortener": {
        "prompt": "Build me a URL shortener in Python. Write it to shortener.py.",
        "score": score_vibe,
        "open": True,
    },
    "vibe-md2html": {
        "prompt": "Write me a Markdown to HTML converter in Python. Write it to md2html.py.",
        "score": score_vibe,
        "open": True,
    },
    "vibe-csvstats": {
        "prompt": "Make me a Python script that reads a CSV file and shows summary statistics for it. Write it to csvstats.py.",
        "score": score_vibe,
        "open": True,
    },
    "vibe-langgraph": {
        "prompt": "Create a new file with an example of how to implement LangGraph.",
        "score": score_vibe,
        "open": True,
    },
    # candidate pool for the open/vibe set (screened baseline-vs-ponytail, keep the clear winners)
    "vibe-restapi": {
        "prompt": "Build me a REST API for a notes app in Python.",
        "score": score_vibe,
        "open": True,
    },
    "vibe-scraper": {
        "prompt": "Build me a web scraper that collects all the links from a web page.",
        "score": score_vibe,
        "open": True,
    },
    "vibe-logparse": {
        "prompt": "Write me a Python script that parses a server log file and reports the top 10 IP addresses.",
        "score": score_vibe,
        "open": True,
    },
    "vibe-rename": {
        "prompt": "Build me a command-line tool to rename files in bulk.",
        "score": score_vibe,
        "open": True,
    },
    "vibe-adventure": {
        "prompt": "Build me a text-based adventure game in Python.",
        "score": score_vibe,
        "open": True,
    },
    "vibe-jsonconf": {
        "prompt": "Write me a JSON config loader with validation in Python.",
        "score": score_vibe,
        "open": True,
    },
    ...
    "tmpl-fe-colorpicker": {
        "prompt": "Add a color picker component to the frontend.",
        "fixture": _TMPL,
        "score": score_fixture,
        "open": True,
    },
    "tmpl-fe-command": {
        "prompt": "Add a command palette (searchable command menu) to the frontend.",
        "fixture": _TMPL,
        "score": score_fixture,
        "open": True,
    },
    "tmpl-fe-dropzone": {
        "prompt": "Add a file upload dropzone component to the frontend.",
        "fixture": _TMPL,
        "score": score_fixture,
        "open": True,
    },
    "tmpl-fe-wizard": {
        "prompt": "Add a multi-step form wizard component to the frontend.",
        "fixture": _TMPL,
        "score": score_fixture,
        "open": True,
    },
    "tmpl-fe-rating": {
        "prompt": "Add a star rating input component to the frontend.",
        "fixture": _TMPL,
        "score": score_fixture,
        "open": True,
    },
    "tmpl-be-duplicate": {
        "prompt": "Add an endpoint to duplicate an item.",
        "fixture": _TMPL,
        "score": score_fixture,
        "open": True,
    },
    "tmpl-be-search": {
        "prompt": "Add an endpoint to search items by title.",
        "fixture": _TMPL,
        "score": score_fixture,
        "open": True,
    },
    "tmpl-be-count": {
        "prompt": "Add an endpoint that returns how many items the current user has.",
        "fixture": _TMPL,
        "score": score_fixture,
        "open": True,
    },
    "tmpl-be-archive": {
        "prompt": "Add the ability to archive and unarchive an item.",
        "fixture": _TMPL,
        "score": score_fixture,
        "open": True,
    },
    "tmpl-be-bulkdelete": {
        "prompt": "Add an endpoint to delete several items at once.",
        "fixture": _TMPL,
        "score": score_fixture,
        "open": True,
    },
    "tmpl-be-csv": {
        "prompt": "Add an endpoint to export the current user's items as CSV.",
        "fixture": _TMPL,
        "score": score_fixture,
        "open": True,
    },
