...
def parse_score(text):
    ...
    try:
        ...
        if "over_engineering" in d:
            d["over_engineering"] = int(d["over_engineering"])
        return d
    except Exception:
        return None
...
def selftest(key):
    ...
    for task_id in ("cache", "safe-path"):
        ...
        if not (
            isinstance(hi.get("over_engineering"), int)
            and isinstance(lo.get("over_engineering"), int)
...
def run(run_dir, key):
    ...
    for i, (tid, arm, model, ws) in enumerate(cells, 1):
        ...
        rec = {
            ...
            "arm": arm,
            "model": model,
            "over_engineering": s.get("over_engineering"),
            "why": s.get("why", ""),
            "cite": s.get("cite", ""),
        }
        scored.append(rec)
    ...
    for r in scored:
        if isinstance(r["over_engineering"], int):
            by_arm[r["arm"]].append(r["over_engineering"])
    ...
    for arm in ["baseline", "caveman", "ponytail", "yagni", "yagni-oneliner"]:
        v = by_arm.get(arm, [])
        if v:
            print(f"  {arm:16} {len(v):>4} {sum(v) / len(v):>6.2f} {max(v):>4}")
    worst = sorted(
        [r for r in scored if isinstance(r["over_engineering"], int) and r["over_engineering"] >= 2],
