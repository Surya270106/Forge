...
NO_RUN = (
    "Write the implementation (include tests if you normally would for a change like this). "
...
def _git(workdir, *args):
    return subprocess.run([shutil.which("git") or "git", *args], cwd=str(workdir), capture_output=True, text=True)
...
def git_diff_stats(workdir):
    ...
    for line in out.splitlines():
        ...
        if Path(path).suffix not in CODE_EXT:
            continue
        if any(k in path for k in _SKIP_DIFF) or "node_modules" in path:
            continue
        n = int(added)
        if _is_test(Path(workdir) / path, Path(workdir)):
            test_loc += n
            test_files += 1
        else:
            loc += n
...
def selftest():
    ...
    for tid, task in TASKS.items():
        if task.get("open"):
            continue  # open tasks measure LOC only, no good/bad refs
        ...
        for kind in ("good", "bad"):
            ...
            print(f"{'ok ' if ok else 'XX '} {tid:12} {kind:4} correct={r['correct']} safe={r['safe']} axis={axis}  {r['reason']}")
...
def _selftest_plugin_dir():
    ...
    try:
        _plugin_dir(missing)
        ok_miss = False  # reached only if it did NOT exit -> broken
    except SystemExit:
        ok_miss = True
...
def run_cell(task_id, arm, model, workdir: Path):
    ...
    if task.get("fixture"):  # copy a real repo in; record what was seeded
        ...
        manifest = sorted(str(p.relative_to(workdir)).replace("\\", "/") for p in workdir.rglob("*") if p.is_file())
    ...
    if task.get("fixture"):
        _git_snapshot(workdir)  # baseline commit -> diff the agent's changes
    ...
    try:
        with open(out_path, "wb") as so, open(err_path, "wb") as se:
            proc = subprocess.Popen(cmd, cwd=str(workdir), stdout=so, stderr=se, start_new_session=(os.name != "nt"))
            try:
        ...
        rows.append(
            {
                ...
                "out_tokens_mean": (
                    round(statistics.mean([c["out_tokens"] for c in cells if c.get("out_tokens") is not None]))
                    if any(c.get("out_tokens") is not None for c in cells)
                    else None
                ),
                "total_tokens_mean": (
                    round(
                        statistics.mean(
                            [
                                ...
                                for c in cells
                                if c.get("out_tokens") is not None
                            ]
                        )
                    )
                    if any(c.get("out_tokens") is not None for c in cells)
                ...
                "time_s_mean": (
                    round(
                        statistics.mean([c["duration_ms"] / 1000 for c in cells if c.get("duration_ms") is not None]),
                        1,
                    )
                    if any(c.get("duration_ms") is not None for c in cells)
                    else None
                ),
            }
        )
...
def print_table(rows):
    ...
    for (task, model), rs in sorted(by.items()):
        ...
        for r in sorted(rs, key=lambda x: x["arm"]):
            ...
            tt = r.get("total_tokens_mean")
            t = r.get("time_s_mean")
            print(
                f"  {r['arm']:16} {r.get('wrote_file_rate', 1.0):>7} {r['correct_rate']:>8} "
...
def main():
    ...
    ap.add_argument(
        ...
        type=int,
        default=4,
        help="cells to run concurrently (default 4; cells are fully isolated)",
    )
    args = ap.parse_args()
    ...
    task_ids = list(TASKS) if args.all else ([t.strip() for t in args.task.split(",")] if args.task else [])
    if not task_ids:
        sys.exit("give --task <id> (comma list ok), --all, or --rescore <dir>")
    ...
    cells = [(tid, arm, model, r) for tid in task_ids for model in models for arm in arms for r in range(args.runs)]
    total = len(cells)
    results, done = [], 0
