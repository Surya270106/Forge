...
from judge import JUDGE_MODEL, RUNS_DIR, judge_call, load_key, parse_score, source_text
...
def _rank_ok(scores):
    ...
    for task_id in sorted({t for (t, _) in scores}):
        ...
        if not (isinstance(hi.get(SCORE_KEY), int) and isinstance(lo.get(SCORE_KEY), int) and hi[SCORE_KEY] > lo[SCORE_KEY]):
            print(f"XX {task_id}: did not rank complete above stub")
            ok = False
        else:
...
PAIRS = [(t, lbl, code) for t in STUBS for lbl, code in (("complete", TASKS[t]["good"]), ("stub", STUBS[t]))]
...
def run(run_dir, key):
    ...
    for i, (tid, arm, model, ws) in enumerate(cells, 1):
        s = parse_complete(judge_call(TASKS[tid]["prompt"], source_text(ws), key, system=RUBRIC)) or {SCORE_KEY: None}
        scored.append(
            {
                "task": tid,
                "arm": arm,
                "model": model,
                SCORE_KEY: s.get(SCORE_KEY),
                "why": s.get("why", ""),
        ...
        (run_dir / "completeness.json").write_text(
            json.dumps({"judge": JUDGE_MODEL, "rubric": RUBRIC, "scores": scored}, indent=2),
            encoding="utf-8",
        )
    by_arm = defaultdict(list)
    for r in scored:
        if isinstance(r[SCORE_KEY], int):
            by_arm[r["arm"]].append(r[SCORE_KEY])
    print(f"\n=== completeness by arm (judge: {JUDGE_MODEL}, 0=stub .. 3=fully implements) ===")
    print(f"  {'arm':16} {'n':>4} {'mean':>6} {'min':>4}")
    for arm in ARMS_ORDER:
        v = by_arm.get(arm, [])
        if v:
            print(f"  {arm:16} {len(v):>4} {sum(v) / len(v):>6.2f} {min(v):>4}")
    under = sorted(
        [r for r in scored if isinstance(r[SCORE_KEY], int) and r[SCORE_KEY] <= FLAG_AT],
